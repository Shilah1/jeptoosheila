...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT Doctor_id, first_name, email, password FROM doctors WHERE email = ? LIMIT 1");
    ...
    if ($result->num_rows == 1) {
        ...
        if ($password === $row['password']) { // Direct comparison since password is not hashed
            ...
            echo "✅ Login successful! Redirecting to Appointmets...";
            header("refresh:2; url=appoinment_viewss.php"); // Redirect to Medical Records Page
            exit();
        } else {
